...
// Define a function that builds a string of city names. Needs a paramater for assignments array.
const assignedCityNames = (assignments) => {
    ...
    for (const assignment of assignments) {
        ...
        for (const city of cities) {
            if (city.id === assignment.cityId && assignments.length > 1) {
                cityNames += `${city.name} and `
            }
    ...
    cityNames = cityNames.substring(0, cityNames.length - 5)
